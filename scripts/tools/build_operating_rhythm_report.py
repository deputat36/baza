"""
Сборка отчета по регламенту эксплуатации базы знаний.
...
SOURCE_FILE = ROOT / "data" / "drafts" / "operating-rhythm.csv"
BUILD_DIR = ROOT / "build"
OUTPUT_MD = BUILD_DIR / "operating-rhythm-report.md"
OUTPUT_CSV = BUILD_DIR / "operating-rhythm-report.csv"
...
REQUIRED_COLUMNS = [
    ...
    "Критичность",
    "Статус",
    "Блокирует стабильность",
]

VALID_CRITICALITY = {"Высокая", "Средняя", "Низкая"}
VALID_STATUS = {"DRAFT", "CHECK", "READY", "DONE", "SKIP"}
...
def validate_rows(rows):
    problems = []
    if not rows:
        return ["Регламент эксплуатации пустой"]
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        blocker = row.get("Блокирует стабильность", "").strip().lower()
        if blocker and blocker not in VALID_BLOCKER:
            problems.append(f"Строка {row_number}: Блокирует стабильность должно быть yes или no")
...
def is_blocker(row) -> bool:
    return row.get("Блокирует стабильность", "").strip().lower() == "yes"
...
def build_report(rows):
    period_counts = Counter(row["Период"] for row in rows)
    ...
    for row in rows:
        by_period[row["Период"]].append(row)
    ...
    lines = [
        ...
        f"- Блокируют стабильность: {len(blockers)}",
    ...
    if not open_blockers:
        ...
        for row in open_blockers:
            lines.append(
                f"- {row['ID']} | {row['Период']} | {row['Задача']} | "
                f"роль: {row['Роль']} | артефакт: {row['Артефакт']}"
            )

    lines.extend(["", "## Детально по периодам", ""])
    for period, period_rows in by_period.items():
        lines.append(f"### {period}")
        lines.append("")
        for row in period_rows:
            lines.append(
                f"- {row['ID']} | {row['Задача']} | {row['Роль']} | {row['Критичность']} | "
    ...
    lines.extend([
        ...
        "",
        "База считается рабочей только если задачи с `Блокирует стабильность = yes` регулярно выполняются и фиксируются в закрытой рабочей таблице.",
        "Публичный репозиторий не должен хранить реальные контакты сотрудников, клиентские данные и внутренние комментарии.",
        "",
    ])
...
def main():
    ...
    if problems:
        print("Регламент эксплуатации содержит ошибки.\n")
