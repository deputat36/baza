...
EXPECTED_ARTIFACTS = [
    ...
    ("launch-decision-log.md", "Журнал управленческого решения запуска"),
    ("launch-decision-log.csv", "Табличная версия журнала решения запуска"),
    ...
    ("source-trust-report.md", "Отчет по уровням доверия источников"),
    ("source-trust-report.csv", "Табличная версия уровней доверия источников"),
    ("record-lifecycle-report.md", "Отчет по жизненному циклу записей"),
    ...
    ("office-launch-packet.md", "Единый пакет проверки перед запуском базы"),
...
MANUAL_CHECKS = [
    ...
    "Открыть `launch-decision-log.md`: проверить, что решение запуска зафиксировано или оставлено как PENDING.",
    ...
    "Открыть `weekly-manager-digest.md`: проверить ключевые действия менеджера на неделю.",
    "Открыть `manager-action-register.md`: назначить ответственных по открытым действиям.",
    "Открыть `source-trust-report.md`: проверить уровни доверия источников и правила использования записей.",
    "Открыть `record-lifecycle-report.md`: проверить статусы записей и правило READY.",
    "Открыть `department-routing-report.md`: проверить маршруты к кураторам центрального офиса.",
    "Открыть `private-contact-schema-report.md`: проверить поля закрытого реестра контактов.",
    "Открыть `contact-verification-report.md`: проверить регламент актуализации контактов.",
    ...
    "Открыть `usage-metrics-report.md`: проверить метрики использования и запросы без результата.",
    "Открыть `adoption-improvement-plan.md`: назначить действия по метрикам и обратной связи.",
    ...
    "Проверить `pipeline-health-report.md`: нет ли рассинхронизации Makefile, workflow и генераторов.",
...
def build_summary():
    ...
    lines.extend([
        ...
        "",
        "Данные можно переносить в закрытую Google Таблицу, если блокирующие проверки прошли, GO/NO-GO отчет не показывает блокеров, решение запуска зафиксировано или явно оставлено как PENDING, XLSX открывается, HTML-превью просматривается, отчёт готовности разобран, владельцы критичных разделов назначены, приемочные сценарии по ролям пройдены, обучение по ролям проведено, еженедельный дайджест менеджера сформирован, реестр управленческих действий разобран, уровни доверия источников заданы, жизненный цикл записей понятен, маршруты к департаментам центрального офиса подготовлены для закрытой таблицы, схема закрытого реестра контактов проверена, регламент актуализации контактов задан, единый пакет запуска сформирован, чек-лист запуска закрыт, регламент эксплуатации понятен, метрики использования заведены, план улучшений после запуска сформирован, план листов, план валидаций, план оформления и план импорта понятны, pipeline-health не показывает критичных рассинхронизаций, а замечания из диагностических отчётов проверены вручную.",
        "",
        "## Ограничение",
        "",
