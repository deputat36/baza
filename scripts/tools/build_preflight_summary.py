...
def build_summary():
    ...
    lines.extend([
        ...
        "",
        "Данные можно переносить в закрытую Google Таблицу, если блокирующие проверки прошли, XLSX открывается, HTML-превью просматривается, отчёт готовности разобран, владельцы критичных разделов назначены, приемочные сценарии по ролям пройдены, обучение по ролям проведено, чек-лист запуска закрыт, регламент эксплуатации понятен, план листов, план валидаций, план оформления и план импорта понятны, а замечания из диагностических отчётов проверены вручную.",
        "",
        "## Ограничение",
        "",
