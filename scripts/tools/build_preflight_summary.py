...
def build_summary():
    ...
    lines.extend([
        ...
        "",
        "Данные можно переносить в закрытую Google Таблицу, если блокирующие проверки прошли, GO/NO-GO отчет не показывает блокеров, решение запуска зафиксировано или явно оставлено как PENDING, XLSX открывается, HTML-превью просматривается, отчёт готовности разобран, владельцы критичных разделов назначены, приемочные сценарии по ролям пройдены, обучение по ролям проведено, единый пакет запуска сформирован, чек-лист запуска закрыт, регламент эксплуатации понятен, план листов, план валидаций, план оформления и план импорта понятны, а замечания из диагностических отчётов проверены вручную.",
        "",
        "## Ограничение",
        "",
