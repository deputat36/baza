"""
Сборка отчета по обучению сотрудников работе с базой знаний.
...
SOURCE_FILE = ROOT / "data" / "drafts" / "role-training-plan.csv"
BUILD_DIR = ROOT / "build"
OUTPUT_MD = BUILD_DIR / "role-training-report.md"
OUTPUT_CSV = BUILD_DIR / "role-training-report.csv"
...
def build_report(rows):
    ...
    lines = [
        ...
        "## Сводка",
        "",
        f"- Всего блоков обучения: {len(rows)}",
        f"- Блокируют запуск: {len(blockers)}",
        f"- Открытых блокирующих блоков: {len(open_blockers)}",
        "",
        "## По ролям",
        "",
    ]
    ...
    lines.extend(["", "## Открытые блокирующие блоки", ""])
    if not open_blockers:
        lines.append("Открытых блокирующих блоков обучения нет.")
    else:
        for row in open_blockers:
            lines.append(
                f"- {row['ID']} | {row['Роль']} | {row['Блок обучения']} | "
                f"задание: {row['Практическое задание']}"
            )
    ...
    for role, role_rows in by_role.items():
        ...
        for row in role_rows:
            lines.append(
                f"- {row['ID']} | {row['Блок обучения']} | уметь: {row['Что должен уметь']} | "
                f"практика: {row['Практическое задание']} | {row['Статус']} | "
    ...
    lines.extend([
        "## Правило обучения",
        "",
        "Офисный запуск нельзя считать готовым, пока блоки с `Блокирует запуск = yes` не пройдены ключевыми ролями и не подтверждены в закрытой рабочей таблице.",
        "",
    ])
...
def main():
    ...
    if problems:
        print("План обучения содержит ошибки.\n")
