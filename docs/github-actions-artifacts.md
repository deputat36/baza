...
6. Открыть `import-plan.md` и сверить порядок переноса.
7. Открыть `data-report.md` и оценить наполненность разделов.
8. Открыть `source-coverage-report.md` и проверить CSV, которые не входят в XLSX.
9. Открыть `id-registry.md` и проверить возможные дубли ID.
10. Открыть `schema-report.md` и проверить заголовки CSV.
